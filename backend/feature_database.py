...
from spatial_grid import SpatialGrid, GridCell
...
class FeatureDatabase:
    ...
    def compute_features_for_area(self, graph: nx.MultiGraph, 
        ...
        for grid_coords, cell in self.spatial_grid.grid.items():
    ...
    def get_cell_features(self, lat: float, lon: float) -> Optional[CellFeatures]:
        """
    ...
    def _compute_elevation_variety(self, cell: GridCell, graph: nx.MultiGraph) -> float:
        ...
        if len(elevations) < 2:
            ...
            if len(cell.nodes) > 0:
                ...
                node_density = len(cell.nodes) / max(1, len(cell.nodes))
